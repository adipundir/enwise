...
export function registerInvoiceTools(server: McpServer) {
  ...
  server.registerTool(
    "delete_invoice",
    {
      ...
      description:
        "Soft-delete an invoice (any status). The merchant has full control over their own invoices. Delete pulls the share page entirely (404 for the recipient) — that's drastic for an invoice that's already been delivered, so for sent / paid / void invoices prefer void_invoice (it keeps the audit record visible to the recipient and sends a void-notification email). Use delete when you want the invoice gone everywhere: drafts that were created by mistake, abandoned tests, or sent invoices the merchant wants to fully retract without leaving a void breadcrumb. Soft-deleted invoices stay in the database for recovery but are excluded from list_invoices and the dashboard. Returns a small status object, NOT the invoice; nothing to display except confirmation.",
      inputSchema: {
    business_id: uuid.optional(), invoice_id: uuid },
    },
    async (args, extra) => {
