...
const createSchema = {
  ...
  /** Omit for the default (issue date + net-30). Pass null for no due date. */
   ...
   *  `[8453, 42161, 1]` for Base, Arbitrum, and Ethereum). Omit to use the business's
...
export function registerInvoiceTools(server: McpServer) {
  ...
  const overrideBank = z
    .object({
      ...
      upi_id: z.string().max(256).nullish(),
