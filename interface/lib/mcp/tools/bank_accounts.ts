 ...
 * "Account number: 0x…" on the share page AND left businesses.evm_wallet_address
...
function rejectIfWalletShaped(fields: {
  ...
  if (an && EVM_ADDRESS.test(an)) {
    return toolError(
      ...
      {
        hint: `Call update_business_profile({ evm_wallet_address: "${an}" }) instead. The Pay-with-USDC button reads businesses.evm_wallet_address — not bank account fields. Storing the wallet here also renders incorrectly on the share page as "Account number: 0x…".`,
      },
    );
  }
  ...
  if ((label && WALLET_LABEL_HINTS.test(label)) || WALLET_LABEL_HINTS.test(holder)) {
    return toolError(
      ...
      {
        hint: "For onchain payouts, call update_business_profile({ evm_wallet_address: '0x...' }). If you really meant a fiat account, rename the label so it doesn't mention 'wallet' or 'USDC'.",
      },
    );
  }
  return null;
