...
export function registerBusinessTools(server: McpServer) {
  ...
  server.registerTool(
    "update_business_profile",
    {
      ...
      description:
        "Update any subset of a business profile (name, tax ID, address, invoice number prefix, logo, contact person, per-chain wallets, preferred EVM crypto chain, etc.). Currency lives on the client (default_currency) or per invoice, not on the business. Omitted fields are left unchanged. Pass null to clear a nullable field. To manage BANK ACCOUNTS, use the dedicated tools — add_bank_account / update_bank_account / remove_bank_account / set_default_bank_account / list_bank_accounts (a business can have multiple bank accounts; one is marked default for new invoices). `contact_name` is the person at the business who handles invoicing — used in PDF letterhead / email footer.\n\n**LOGO UPLOAD — preferred path for local files (fastest):** call `request_attachment_upload({filename: 'logo.png', mime_type: 'image/png'})` to get a presigned PUT URL, curl the file bytes to that URL, then pass the returned blob URL here as `logo: { image_url: '<blob-url>' }`. The server detects Vercel Blob URLs and skips the re-upload — this is near-instant. For a logo already at a public URL: `logo: { image_url: 'https://…' }` — server fetches and stores it (slower). DO NOT base64-encode a local file — it bloats the payload and is slow for anything over 50 KB. The `image_base64` path is only for tiny programmatically generated images.\n\n**WALLET — one field covers all EVM chains.** `evm_wallet_address`: raw 0x + 40 hex or ENS `name.eth`. One address receives USDC on Base, Arbitrum, Ethereum, Polygon, and all other EVM chains. The Pay-with-USDC button reads this field. When the user pastes a 0x address or says 'set my wallet', save it here — do NOT ask about Starknet or Aptos. DO NOT call add_bank_account with a wallet.\n\n**CHAINS** — `accepted_chain_ids` is the set of EVM chains the payer can choose from on the share page. Default value: `[8453, 42161, 1]` (Base, Arbitrum, and Ethereum all on by default). `payment_chain_id` is the chain pre-selected for the payer (defaults to Base 8453, so payers are not defaulted onto pricier Ethereum gas). Supported ids: 8453 Base mainnet (USDC), 42161 Arbitrum One (USDC), 1 Ethereum mainnet (USDT). Note: Ethereum mainnet gas is pricey, so a payer who picks it pays more — it is offered by default but not pre-selected. Pass null to reset to defaults. RULE: do NOT narrow `accepted_chain_ids` to a single chain unless the user explicitly says they only want one — keep the default multi-chain set unless told otherwise. Never tell the user crypto is 'not configured' if they have an EVM wallet set. Pass `business_id` if the user owns multiple businesses.",
