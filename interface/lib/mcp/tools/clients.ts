...
export function registerClientTools(server: McpServer) {
  server.registerTool(
    "create_client",
    {
      title: "Create client",
      description:
        "Add a new client using details the user has explicitly given you. NEVER invent a client name, email, address, tax ID, or wallet address. If the user hasn't told you these, ASK in ONE batched message (name + email + full address as a single freeform paste) — do not drip-feed questions and do not ask the user to type address parts separately. YOU split their pasted address into `address_line1` / `city` / `region` / `postal_code` / `country` (ISO-3166 alpha-2) before calling this tool. `name` is the legal entity (the company billed); `contact_name` is the optional human at that company who'll receive the email — used as the email greeting (\"Hi Aditya,\"). For sole proprietors / freelancers, `contact_name` can equal `name`. `wallet_address` is the client's onchain identity (raw 0x… or ENS name like `acme.eth`) — surfaced on the invoice when present. Pass `business_id` when the user owns multiple businesses. Returns the created client including its id. Remember the id for follow-up tool calls in this session.",
