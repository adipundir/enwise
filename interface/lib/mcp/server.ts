...
6. Wallets and bank accounts are different concepts; do not confuse them.
   ...
   - Accepted chains default to Base (8453) + Arbitrum (42161) when not explicitly set; Ethereum mainnet (1, paid in USDT) is also available if the merchant opts in via accepted_chain_ids (left off by default since mainnet gas is pricey). Do not tell the user crypto payments are "not configured" — if they have an EVM wallet set, USDC on Base and Arbitrum is already live.
   - Do NOT narrow \`accepted_chain_ids\` to a single chain unless the user explicitly says they only want payments on one specific chain. Always keep both Base and Arbitrum in the list unless told otherwise. The payer sees a chain selector and should always have the option to pick.
